...
fit_state <- function(state_dat, sh_vec){ # no seeding for 1D optimization
  #sink("state.log", append=TRUE)
  #print(state_dat)
  oo <- optimize(ss_wrapper, c(0, 1), st_e=state_dat, sh_p=sh_vec)
  #cat("Done for idx: ", state_dat$idx)
  #sink()
...
for (iter in seq(20)){
  ...
  sh_fit <- psoptim((seed_sh-sh_low)/(sh_high-sh_low), all_state_negLL, ss_prms=ss_fit,
                    ...
                    control=list(trace=1, REPORT=5, maxit=4000, trace.stats=FALSE, maxit.stagnate=200))
  seed_sh <- sh_low + sh_fit$par*(sh_high-sh_low)
  
  cat(">>", R0_mod, "Iter", iter, "negLL:", sh_fit$value, "\n")
  #cat("h_rate; R0_min; R0_range; [R0_prms]\n")
  cat(">> ", unlist(ss_fit), "\n>> ", seed_sh, "\n")
  save(ss_fit, seed_sh, file = paste0("fit_results/", handle,"_iter",iter, "_iterative.city.glob.fit1.clean3.rds"))
}

save(ss_fit, seed_sh, file = paste0("fit_results/", handle, "_iterative.city.glob.fit1.clean3.rds"))
stopCluster(clstr)
