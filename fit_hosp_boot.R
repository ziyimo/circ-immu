...
if (FALSE){
  ...
  boot_params <- matrix(0, nrow = 0, ncol = 14)
  for (batch in seq(0, 9)){
    prms_mtx <- readRDS(paste0("fit_results/sd.boot.eta.05_seeded", batch, ".rds"))
    boot_params <- rbind(boot_params, prms_mtx)
  }
  
  saveRDS(boot_params, file = "covid_hosp_fit/sd.boot.eta.05_seeded.rds")
}