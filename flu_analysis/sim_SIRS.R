...
mode <- args[3]
bestfit_path <- args[4]
resamp_path <- args[5] # depends on `mode`

handle <- paste0(state_ls, l_pnl, R0_mod, "_", mode, "sims")

fitted <- readRDS(bestfit_path)
prms_optim <- unname(fitted$optim$bestmem[-1]) # do not include `c`

if (mode == "hess"){
  library(MASS)
  ## Hessian ##
  cov_Mtx <- readRDS(resamp_path) # read covariant matrix
  resamp <- mvrnorm(n=10, prms_optim, cov_Mtx) # 10 replicates for now
} else if (mode == "boot") {
  ## Bootstrap ##
  resamp <- readRDS(resamp_path) # resampled parameters
}
