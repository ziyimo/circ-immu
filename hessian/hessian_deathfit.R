...
seed_file <- args[3]
discret <- args[4] # 0.005 step for gradient approximation, path to sampled parameters when below is -1
grad2hess <- args[5] # 1 this is the parscale parameter in optimHess, put -1 for evaluating sampled parameters
nthr <- args[6]

# text file with a list of states to fit to
load(seed_file) # seed_sh and ss_fit
### Logging
...
# remove death param at position 10
# position is no_cdv + 1
seed_sh <- seed_sh[-(no_cdv +1)]
...
# ss_prms are no longer used instead they are fixed to ss_fit
## Function for fitting shared parameters
all_state_negLL <- function(orig_prms){
  #orig_prms <- sh_low + norm_sh_prms*(sh_high-sh_low)
  # pop in fixed death rate

  #sink("hess.log")
  #print(orig_prms)
  # workaround to exclude death from params as its fixed
  #orig_prms <- c(orig_prms[(1:no_cdv)],c(0.001),orig_prms[(no_cdv+1:length(orig_prms))])
  r0idx <- no_cdv + 1
  prmlen = length(orig_prms)
  orig_prms <- c(orig_prms[(1:no_cdv)],c(0.001),orig_prms[(r0idx:prmlen)])
  #print(orig_prms)
  #sink()
  states_negLL <- parSapply(cl = clstr, X=city_data, FUN=sh_wrapper, sh_orig=orig_prms, ss_ls=ss_fit)
  # sum changed to be negative for hessian
  return(-sum(states_negLL))
}
sh_wrapper <- function(st_e, sh_orig, ss_ls){
  ss_p <- ss_ls[[st_e$idx]]
  #sink("shwrp.log",append = TRUh)
  #print(st_e)
  #print(ss_p)
  #sink()
...
#-optimHess(seed_sh, all_state_negLL,control = list(parscale=rep(grad2hess, length(seed_sh)), ndeps=hessian_step))
...
if (grad2hess == -1){
  ...
  cat(">>", hessian_step, "\n")
  cat(">> grad2hess ratio:", grad2hess, "\n")
  info_mtx <- -optimHess(seed_sh, all_state_negLL,
  ...
  cat(">>", R0_mod, ";", grad2hess, discret, ":", diag(cov_mtx), "\n")
  
  saveRDS(cov_mtx, file=paste0("fit_results/",handle, "_", grad2hess, "g", discret, "_covMtx.rds"))
