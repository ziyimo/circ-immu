...
load(args[3])  # load seed_sh
init_inf <- as.numeric(args[4])
target_state <- args[5]
covmtx <- readRDS(args[6])
...
city_data <- list()
seed_ss <- list()
# Remove death rate as this was fixed
...
state_wrapper_resamp <- function(state_elem, ss_prms, sh_prms, target_state,index,old_seed){
  if (R0_mod %in% c("sun", "sd", "hsd")){
    # Init, alpha,R0min,R0range,s_0,other
    state_prms <- c(ss_prms, sh_prms[(no_cdv+1):(no_cdv+3+s0_idx-1)], sh_prms[state_elem$cdv], sh_prms[-1:-(no_cdv+3+s0_idx-1)])
    cat(state_prms,"\n")
    old_seed <- c(ss_prms, old_seed[(no_cdv+1):(no_cdv+3+s0_idx-1)], old_seed[state_elem$cdv], old_seed[-1:-(no_cdv+3+s0_idx-1)])
  } else {
    state_prms <- c(ss_prms, sh_prms)
  }
  neg_LL <- binom_seird_hess(state_prms, 
                   paste0("R0_", R0_mod),
                   state_elem$var,
                   state_elem$epi,
                   state_elem$pop,
...
reseed_sh <- seed_sh[-(no_cdv +1)]
resamp <- mvrnorm(n=10, reseed_sh, covmtx)

for (row in 1:nrow(resamp)) {
 rseed <- as.numeric(resamp[row,])
# add back death rate
 lenrseed <- length(rseed)
 midstart <- no_cdv+1
 rseed <- c(rseed[1:no_cdv],c(0.001),rseed[midstart:lenrseed])
 print(rseed)
 state_wrapper_resamp(get(target_state, city_data),init_inf,rseed, target_state,row,seed_sh)
 }
